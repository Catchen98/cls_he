...
if __name__ == '__main__':
    ...
    # IMAGE_FOLDER = args.root + '/ICPR/Charts/'
    ...
    train = pd.read_csv(args.root + '/PMC_train.csv')
    test = pd.read_csv(args.root + '/PMC_test.csv')
    # train = pd.read_csv(args.root + '/synthetic_train.csv')
    # test = pd.read_csv(args.root + '/synthetic_test.csv')
    if args.debug:
        logger.info('=> debug..')
        train = train.sample(n=120)
   
    train_labels = train.loc[:, 'area':'vertical_interval']
    # train_labels = train.loc[:, 'area':'vStack']
    train_image_id = train['image_id'].tolist()
    ...
    def get_image_path(filename):
        ...
        # print(filename)
        # return (IMAGE_FOLDER +  type_list[train_labels.loc[train_image_id.index(filename)].tolist().index(1)]+ '/' + str(filename) + '.png')
        # if os.path.exists(IMAGE_FOLDER + str(filename) + '.png'):
        #     return (IMAGE_FOLDER + str(filename) + '.png')
        # else:
        #     return (IMAGE_FOLDER + filename + '.jpg')
