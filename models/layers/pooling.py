...
class AdaptiveGeM2d(nn.Module):
    ...
    def __init__(self, output_size=(1,1),p=3.0, eps=1e-6, freeze_p=True, clip_max=None):
    ...
    def forward(self, x):
        # return LF.gem(x, p=self.p, eps=self.eps)
        return adaptive_gem2d(x, self.output_size,p=self.p, eps=self.eps, clip_max=self.clip_max)
...
def get_adaptive_pooling(pool_type, output_size=(1,1)):
    gap = None
    if pool_type == "avg":
        gap = nn.AdaptiveAvgPool2d(output_size=output_size)
    elif pool_type == 'max':
        gap = nn.AdaptiveMaxPool2d(output_size=output_size)
    elif "gem" in pool_type:
        if pool_type !='gem':
            p = pool_type.split('_')[-1]
            p = float(p)
            gap = AdaptiveGeM2d(output_size=output_size, p=p, eps=1e-6, freeze_p=True, clip_max=1e8)
        else:
            gap = AdaptiveGeM2d(output_size=output_size, eps=1e-6, freeze_p=False, clip_max=1e8) # clip_max to support convert2keras
    return gap 