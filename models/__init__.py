...
def build_model(cfg, num_classes):
    if cfg.MODEL.NAME == "baseline":
        model = Baseline(num_classes, cfg.MODEL.LAST_STRIDE, cfg.MODEL.PRETRAIN_PATH,
                            backbone=cfg.MODEL.BACKBONE,pool_type=cfg.MODEL.BASELINE.POOL_TYPE, \
                            use_dropout=cfg.MODEL.USE_DROPOUT, use_bnneck=cfg.MODEL.BASELINE.USE_BNNECK, cls_bias=cfg.MODEL.BASELINE.USE_CLS_BIAS, \
                            use_bnbias=cfg.MODEL.BASELINE.USE_BNBAIS
                            )
        ...
        model = MGNBNNeck(num_classes, cfg.MODEL.PRETRAIN_PATH,last_stride=cfg.MODEL.LAST_STRIDE, backbone=cfg.MODEL.BACKBONE,pool_type=cfg.MODEL.MGN.POOL_TYPE,\
                    use_center=cfg.SOLVER.CENTER_LOSS.USE,\
                    num_share_layer3 = cfg.MODEL.MGN.NUM_SHARE_LAYER3)
    elif cfg.MODEL.NAME == 'delf':
        model = DELF(num_classes, cfg.MODEL.LAST_STRIDE, cfg.MODEL.PRETRAIN_PATH, \
                            backbone=cfg.MODEL.BACKBONE, target_layer='layer3', l2_norm_att=True, \
                            use_dropout=cfg.MODEL.USE_DROPOUT,\
                            cosine_loss_type=cfg.MODEL.BASELINE.COSINE_LOSS_TYPE,s=cfg.MODEL.BASELINE.S,m=cfg.MODEL.BASELINE.M,\
                            use_bnbias=cfg.MODEL.BASELINE.USE_BNBAIS,use_sestn=cfg.MODEL.BASELINE.USE_SESTN, use_bnneck=cfg.MODEL.BASELINE.USE_BNNECK)
