...
class EfficientNet(nn.Module):
    ...
    def __init__(self, width_mult=1.0, depth_mult=1.0, dropout_rate=0.2, num_classes=1000):
        ...
        features_0 = [ConvBNReLU(3, out_channels, 3, stride=2)]
        features_1 = []
        features_2 = []
        features_3 = []
        features_4 = []
        ...
        for index,  (t, c, n, s, k) in enumerate(settings):
            ...
            for i in range(repeats):
                ...
                if index <= 1:
                    features_0 += [MBConvBlock(in_channels, out_channels, expand_ratio=t, stride=stride, kernel_size=k)]
                elif index <= 2:
                    features_1 += [MBConvBlock(in_channels, out_channels, expand_ratio=t, stride=stride, kernel_size=k)]
                elif index <= 4:
                    features_2 += [MBConvBlock(in_channels, out_channels, expand_ratio=t, stride=stride, kernel_size=k)]
                elif index <= 5:
                    features_3 += [MBConvBlock(in_channels, out_channels, expand_ratio=t, stride=stride, kernel_size=k)]
                elif index <= 6:
                    features_4 += [MBConvBlock(in_channels, out_channels, expand_ratio=t, stride=stride, kernel_size=k)]
                in_channels = out_channels
            if index == 1:
                self.layer0 = nn.Sequential(*features_0)
            if index == 2:
                self.layer1 = nn.Sequential(*features_1)
            if index == 4:
                self.layer2 = nn.Sequential(*features_2)
            if index == 5:
                self.layer3 = nn.Sequential(*features_3)
            if index == 6:
                self.layer4 = nn.Sequential(*features_4)
